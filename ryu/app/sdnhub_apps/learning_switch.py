...
class L2LearningSwitch(app_manager.RyuApp):
    ...
    def is_packet_exempted(self, pkt):
        ...
        if eth.ethertype == ether.ETH_TYPE_ARP:
            ...
            if ip_hdr.proto == inet.IPPROTO_TCP:
                ...
                tcp_hdr = pkt.get_protocols(udp.udp)[0]
                fields['tp_src'] = tcp_hdr.src_port
                fields['tp_dst'] = tcp_hdr.dst_port
    ...
    def packet_in_handler(self, ev):
        ...
        if eth.ethertype == 35020:
            return
        if eth.ethertype is None or eth.ethertype != 2048 and eth.ethertype != 2054 :
            return
        ...
        if out_port != ofp.OFPP_FLOOD:
            match = self.matchFromPacket(msg.match, pkt, ofp_parser) #ofp_parser.OFPMatch(in_port=in_port, eth_dst=dst)
            print("\n\n\n\n\n\n\n\n")
            print(match)
            #match = msg.match
    ...
    def matchFromPacket(self, match, pkt, ofp_parser):
        eth = pkt.get_protocols(ethernet.ethernet)[0]
        dst = eth.dst
        src = eth.src
        eth_type= eth.ethertype
        if(eth_type == 0x0800):
            ipproto = pkt.get_protocols(ipv4.ipv4)[0]
            ipSrc = ipproto.src
            ipDst = ipproto.dst
            ip_proto = ipproto.proto
            if(ip_proto==0x06):
                tcpPkt = pkt.get_protocols(tcp.tcp)[0]
                return ofp_parser.OFPMatch(in_port=match['in_port'], eth_dst=dst, eth_src=src, eth_type=eth_type, ipv4_src=ipSrc, ipv4_dst=ipDst, ip_proto=ip_proto, tcp_src=tcpPkt.src_port, tcp_dst=tcpPkt.dst_port)
            elif(ip_proto ==0x11):
                udppkt = pkt.get_protocols(udp.udp)[0]
                return ofp_parser.OFPMatch(in_port=match['in_port'], eth_dst=dst, eth_src=src, eth_type=eth_type, ipv4_src=ipSrc, ipv4_dst=ipDst, ip_proto=ip_proto, udp_src=udppkt.src_port, udp_dst=udppkt.dst_port)

            return ofp_parser.OFPMatch(in_port=match['in_port'], eth_dst=dst, eth_src=src, eth_type=eth_type, ipv4_src=ipSrc, ipv4_dst=ipDst, ip_proto=ip_proto)
        else:
            return ofp_parser.OFPMatch(in_port=match['in_port'], eth_dst=dst, eth_src=src, eth_type=eth_type)
